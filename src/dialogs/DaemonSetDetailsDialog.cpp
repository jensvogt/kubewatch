...
namespace {
    ...
    // present in the pod's labels -- true here of the daemonset's own pod
...
void DaemonSetDetailsDialog::Show(QWidget *parent, const QStringList &baseArgs, const QString &name, const QString &ns) {
    ...
    const QJsonObject spec = daemonSet["spec"].toObject();
    const QJsonObject status = daemonSet["status"].toObject();
    ...
    dialog.setWindowTitle("DaemonSet: " + ns + "/" + name);
    ...
    podsStatusForm->addRow("Desired", new QLabel(QString::number(status["desiredNumberScheduled"].toInt())));
    podsStatusForm->addRow("Current", new QLabel(QString::number(status["currentNumberScheduled"].toInt())));
    podsStatusForm->addRow("Ready", new QLabel(QString::number(status["numberReady"].toInt())));
    podsStatusForm->addRow("Updated", new QLabel(QString::number(status["updatedNumberScheduled"].toInt())));
    podsStatusForm->addRow("Available", new QLabel(QString::number(status["numberAvailable"].toInt())));
    podsStatusForm->addRow("Unavailable", new QLabel(QString::number(status["numberUnavailable"].toInt())));
